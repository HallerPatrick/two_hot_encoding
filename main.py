...
def run_train(args, writer=None, no_run=None):
    ...
    if args.model == 'Transformer':
        model = _model.TransformerModel(ntokens, args.emsize, args.nhead, args.nhid, args.nlayers, args.dropout).to(device)
    else:
        model = _model.RNNModel(args.model, ntokens, args.emsize, args.nhid, args.nlayers, args.dropout, args.tied, args.only_unigrams).to(device)
    ...
    if writer:
        writer.add_hparams(vars(args), {"hparam/test loss": test_loss, "hparam/ppl": math.exp(test_loss)})
        writer.add_text("lstm/{}".format(no_run), str(vars(args)) + "\n\n------\n" + gen(model, corpus, device))
